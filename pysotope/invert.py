...
def gen_one_interf_func(m_corr, m_ref, interf_elem, file_spec):
...
def create_row_lookup(mass):
    def f(row, *args, **kwargs):
        return row[mass]
    return f


def create_interf_corr(m_corr, file_spec):
    interf = file_spec['used_isotopes'][m_corr]
    interf_funcs = dict()
    for m_ref, elem  in interf:
        interf_funcs[m_ref] = gen_one_interf_func(
            m_corr,
            m_ref,
            elem,
            file_spec)
    def f(row, beta):
        i = file_spec['cycle_columns'][m_corr]
        v = row[i]
        for k, fun in interf_funcs.items():
            i = file_spec['cycle_columns'][k]
            v -= fun(row[i], beta)
        return v
    return f
...
def calc_stat(data_in: np.array, label,
    ...
    '''
    Convenience function for caluclating summary statistics.
    '''
    data_out = []
    labels_out = []
    if mean:
