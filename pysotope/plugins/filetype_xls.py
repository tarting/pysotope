'''
Filetype plugin for .xls files for the MicroMass Isoprobe T and IsotopX Phoenix
TIMS mass spectrometers at the University of Copenhagen Isotope Geology lab.
The csv parsing module can be used to process any csv file (currently only comma
separated and with decimal dots). The xls_dump function reads any xls document
given a xls to csv converter given by the converer_path, which dumps the
contents of sheets contiguously to stdout.
'''
...
def distill_to_csv(
    ...
    for k, spec in file_spec.items():
        aggregated += k + '\n'
        data = parse_from_filespec(contents, spec)
        for r in data:
            aggregated += ','.join(r) + '\n'
    return aggregated
